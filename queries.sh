...
PSQL="psql --username=zamyn17 --dbname=worldcup --no-align --tuples-only -c"
...
echo "$($PSQL "select teams.name from teams full join games on teams.team_id = games.winner_id or teams.team_id = games.opponent_id where year = 2014 and round = 'Eighth-Final'")"
...
echo "$($PSQL "select year, name from teams full join games on teams.team_id = games.winner_id where round = 'Final' and team_id = winner_id order by name")"
